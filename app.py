...
def train_prophet_models(data_df):
    ...
    logging.info(f"Enter to Train Model")
    product_ids = data_df['ProductId'].unique()
    logging.info(f"product_ids filled")

    for pid in product_ids:
        logging.info(f"Training model for ProductId: {pid}")
        product_df = data_df[data_df['ProductId'] == pid][['ds', 'y']].copy()
        
        if len(product_df) < 10: 
            logging.info(f"Skipping ProductId {pid} due to insufficient data points: {len(product_df)}")
            continue
        ...
        try:
            ...
            logging.info(f"Model for ProductId: {pid} trained successfully.")
        except Exception as e:
            logging.info(f"Error training model for ProductId {pid}: {e}")
            
    logging.info(f"Trained {len(prophet_models)} models.")
...
if __name__ == '__main__':
    logging.info("Loading data and training models...")
    try:
        main_df = load_and_prepare_data(DATA_URL)
        train_prophet_models(main_df)
        ...
        if 9 in prophet_models:
            ...
            logging.info(sample_forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']])
        ...
        logging.info("Example: http://127.0.0.1:5000/predict?product_id=9&future_date=2024-01-01")
